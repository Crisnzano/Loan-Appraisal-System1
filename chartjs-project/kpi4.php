...
<?php
  ...
  $monthlyCustomerRetention_target = 13000; //$salesPerProduct_target 
...
<?php
  ...
  $sql = "CALL `ExpensesVsRevenue`($currentYear_top5LoanPlans);";
  $result = $conn->query($sql);

  if ($result->num_rows > 0) {
    $row_num = array();
    $planID = array(); //productCode
    $loanRepaymentAmount = array(); //productName
    $loanPercentage = array(); //productLine
    $fullLoanDetails = array(); //fullProductDetails
    $sumRepaymentEach = array(); //sumPriceEach
    // output data of each row
    while($row = $result->fetch_assoc()) {
      $row_num[$row['row_num']] = $row['row_num'];
      $planID[$row['row_num']] = $row['planID'];
      $loanRepaymentAmount[$row['row_num']] = $row['sumRepaymentAmount'];
      $fullLoanDetails[$row['row_num']] = $row['fullPlanDetails'];
      $sumRepaymentEach[$row['row_num']] = $row['sumExpenseEach'];
    }
  } else {
    "0 results";
  }
...
<?php
  ...
  $sql = "CALL `ExpectedLossPerMonth`($currentYear_top5LoanPlans);";
  $result = $conn->query($sql);
...
<div class="col-md-6 my-1">
  ...
  <div class="card-body text-center">
    <strong>
      KPI4a (leading): <u>Expenses vs Revenue</u><br>
      Target for the Annual Customer retention per Loan = <?= number_format($monthlyCustomerRetention_target,2,".",",") ?> <br>
...
<div class="col-md-6 my-1">
  ...
  <div class="card-body text-center">
    <strong>
      KPI4b (lagging): <u>Expected Loss per Month</u><br>
      Target for the Monthly Sales Revenue = <?= number_format($monthlyLoanRevenue_target,2,".",",") ?> <br>
      Current Year = <?= $currentYear_monthlyLoanRevenue?>
    </strong>
  </div>
