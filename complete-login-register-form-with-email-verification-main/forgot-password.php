...
if (isset($_SESSION['SESSION_EMAIL'])) {
    header("Location: .../Loan_Appraisal_System1/Client.php");
    die();
}
...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM roles WHERE email='{$email}'")) > 0) {
        $query = mysqli_query($conn, "UPDATE roles SET code='{$code}' WHERE email='{$email}'");
        ...
        if ($query) {        
            ...
            try {
                ...
                $mail->Password   = 'tccjojxhdacazmgn';                               //SMTP password
